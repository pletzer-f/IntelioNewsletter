...
// Agent 00 (monthly profile) needs its own deliverable spec + the source
// hierarchy — not the section-agent specs or HTML output format. Slicing
// trims ~3k input tokens off every first-run and monthly refresh.
const SOURCE_RULES = SKILL_MD.slice(
  SKILL_MD.indexOf('## 4. SOURCE HIERARCHY'),
  SKILL_MD.indexOf('## 5. EXECUTION WORKFLOW')
).trim();
const AGENT00_SPEC = SKILL_MD.slice(
  SKILL_MD.indexOf('### AGENT 00 - MONTHLY'),
  SKILL_MD.indexOf('### AGENT 01 - MACRO')
).trim();
...
<article class="story-lead">
  ...
  <div class="impl-block">
    ...
    <div class="impl-body"><div class="impl-inner"><strong>Signal:</strong> [exactly one of: direkt operativ / indirekt (Makro) / Kontext-Risiko — or the English equivalents for 'en' clients] · <strong>Evidenz:</strong> [hoch / mittel / niedrig — how strongly the sources support the CLIENT implication, not the news fact itself]. [1–2 sentences client relevance: named entity + direction + horizon. Where simple arithmetic on figures already in this story allows it, add ONE clearly-labelled illustrative range — e.g. "Beispielrechnung: 50 bp weniger auf €100 Mio. variabler Finanzierung ≈ €0,5 Mio. Zinskosten p. a." — never new external figures.] <strong>Risiko zur These:</strong> [1 sentence: the counter-case, downside, or what would invalidate this reading.] <strong>Zu beobachten:</strong> [the CONCRETE observable that would confirm or refute this thesis — a named metric, data release, threshold, or date, e.g. "August-ADR und Stornoquoten vs. Budget; BEG-Auszahlungsquoten im Q3-Bericht; Eurostat-Vollzahlen am 17. Juli". Stay in the analyst's seat: NEVER assign tasks to people or roles, NEVER set internal deadlines, no imperatives like "aufbereiten"/"prüfen Sie" — and never a content-free "beobachten" without naming WHAT.]</div></div>
  </div>
...
The four bold labels inside impl-inner are MANDATORY in every story, written in the client's output language:
DE: Signal / Evidenz / Risiko zur These / Zu beobachten · EN: Signal / Evidence / Risk to thesis / Watch.
TONE: the briefing ANALYSES — it does not manage. No instructions to the client's staff, no role assignments, no deadlines anywhere.]`;
 ...
 *  German cards run ~1,000–1,200 output tokens each; the structured implication
 *  block (Signal/Evidenz/Risiko/Zu beobachten + Beispielrechnung) adds
 *  ~200–300 more. ~1,800/card gives comfortable headroom (you only pay for
 *  tokens actually generated, so the higher ceiling is free). */
function maxTokensFor(count) {
  return Math.min(16000, 2000 + count * 1800);
}
...
${dataSheet ? `\n${dataSheet}\n` : ''}
## ANALYTICAL DISCIPLINE — CEO STANDARD (client feedback, binding)
- SEPARATE FACT FROM INTERPRETATION. The lede/body state what happened (fact, with figure + date). The implication block carries the interpretation — and must be written as interpretation ("kann", "spricht für", "dürfte"), never as established consequence.
- INDICATOR TAXONOMY — market indicators (indices, FX, rates, commodities, gold) are INDIRECT sentiment signals. NEVER present them as direct evidence of operational outcomes (bookings, order intake, occupancy). A one-day index move is a risk-appetite signal, not a demand indicator — say so, and name the operational metric that would actually confirm the thesis (e.g. ADR, occupancy, quote-to-order conversion, permit approvals). Tag every implication with its signal type as specified in the output format.
- QUANTIFY MAGNITUDE where possible. "Positiv"/"unterstützend"/"margin-enhancing" without a size is not CEO-grade. When simple arithmetic on figures already present in the story allows it, give ONE illustrative range, clearly labelled ("Beispielrechnung: …", "illustrativ: …"). Illustrative arithmetic must be derivable from stated figures + elementary math only — it is NOT a licence to introduce new external numbers.
- DATA FRESHNESS — when an official statistic you use is older than ~2 months relative to Today above, say so explicitly in the text (e.g. "letzter offizieller Stand: Dezember 2025") and note what fresher signal would sharpen the view. Never present a lagging figure as the current rate.
${id === '01' ? '' : `- NO MACRO RETELLING. ECB decisions, index moves, FX and commodity levels belong to the Macro section (Agent 01). Do not retell them as stories here; if a macro development drives your sector story, reference it in ONE clause and spend your words on the sector-specific consequence.`}
- INFORMATION GAP PROTOCOL — when the sources are too thin to cover a topic that is clearly central to this client${isReputationAgent ? ' (e.g. local permits, municipal decisions, project-specific coverage)' : ''}, do NOT pad and do NOT stop at "manual research recommended". Instead close the section with ONE compact protocol card (an <article class="story-lead"> whose story-body is a short list): WAS (the concrete registers, municipalities, bodies, or projects from the client context worth tracking), WO (the specific source or database), WIE OFT (a sensible cadence), SIGNAL (what kind of finding would be materially relevant). Frame it as a monitoring blueprint the reader MAY adopt — do not assign it to a person or role, and no deadlines. No key-stat, no source link for this card.
...
Base your analysis on the search results provided. Follow the AGENT 00 deliverable spec below.
 ...
 */
/** Slim the evidence payload for verification: keep full objects only where we
 *  have fetched article text (that's what claims are checked against); everything
 *  else collapses to {title, url} so URL-existence checks still work. Cuts the
 *  verifier's input cost substantially with no loss of checkable evidence. */
function slimForVerification(results) {
  if (!Array.isArray(results)) return results;
  return results.map(r => (r && r.text) ? r : { title: r?.title, url: r?.url });
}
...
- ILLUSTRATIVE ARITHMETIC — implication blocks may contain ONE clearly-labelled example calculation ("Beispielrechnung: …", "illustrativ: …"). These are ALLOWED even though their result is not in the sources, PROVIDED (a) the label is present, (b) every input figure appears in the story or VERIFIED FIGURES, and (c) the arithmetic is correct — verify it; fix wrong math, add the label if missing, delete if the inputs are unsupported.
- STRUCTURED IMPLICATION — the impl-inner labels (Signal / Evidenz|Evidence / Risiko zur These|Risk to thesis / Zu beobachten|Watch) are part of the required format: keep them. Check the Signal tag is honest — an index/FX/rate move tagged "direkt operativ" must be corrected to "indirekt (Makro)". TONE: the briefing analyses, it does not manage — rewrite any sentence that assigns a task to a person/role or sets an internal deadline (e.g. "Treasury: bis Freitag aufbereiten", "Frage an CFO bis …") into an observational watchpoint naming the metric/release instead. An INFORMATION-GAP protocol card (a story listing what/where/how often to check, with no key-stat and no source link) is legitimate — do not delete it for lacking sources.
...
- If NOTHING needs correcting, output exactly OK (those two letters, nothing else) — do NOT re-emit the section.
- Otherwise output ONLY the corrected HTML, starting with <article.`;
  try {
    const out = ((await complete(system, user, opts.maxTokens || 16000, usage, opts)) || '').trim();
    if (out === 'OK') return sectionHtml;                    // clean bill — skip the expensive re-emit
...
- INDICATOR DISCIPLINE (whole summary, not just the box) — a market indicator (index, FX, rate, commodity move) is an INDIRECT sentiment signal: rewrite any sentence that presents one as direct evidence of an operational outcome ("DAX-Sprung signalisiert Nachfragebelebung", "Vorlaufindikator für Buchungen") into hedged sentiment language ("kann auf … hindeuten") that names the operational metric which would actually confirm it.
- PRIORITIES PANEL — the "PRIORITÄTEN HEUTE"/"TODAY'S PRIORITIES" panel after summary-prep is a REQUIRED part of the format. If it is present: verify each topic is grounded in a section, topics are ≤4 words and observables ≤10 words (shorten if longer). If it is MISSING: BUILD it yourself from the sections — 3–4 topics ranked by materiality (chip: HOCH #FEE2E2/#B91C1C · MITTEL #FEF3C7/#B45309 · NIEDRIG #E2E8F0/#475569) × urgency (left bar: now #C41E3A · this week #D97706 · ongoing #94A3B8), using this exact structure per row inside a bordered, rounded <div> with an uppercase header line and a closing micro-legend ("Farbleiste links = Dringlichkeit · Chip = Materialität"):
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse"><tr><td width="4" style="background:[urgency colour]"></td><td style="padding:10px 14px 10px 12px;border-bottom:1px solid var(--surface-hi,#F3F0EB)"><span style="font-family:'Playfair Display',Georgia,serif;font-size:16px;font-weight:800;color:var(--accent,#C41E3A)">[rank]</span>&nbsp; <strong style="font-family:'Inter',Arial,sans-serif;font-size:13.5px;color:var(--text,#0F172A)">[topic ≤4 words]</strong><div style="font-family:'Inter',Arial,sans-serif;font-size:11.5px;color:var(--text-2,#475569);margin-top:2px">[observable ≤10 words]</div></td><td align="right" style="padding:10px 14px;border-bottom:1px solid var(--surface-hi,#F3F0EB);white-space:nowrap"><span style="display:inline-block;padding:3px 10px;border-radius:20px;font-family:'Inter',Arial,sans-serif;font-size:9.5px;font-weight:700;background:[chip bg];color:[chip colour]">[HOCH|MITTEL|NIEDRIG]</span></td></tr>…</table>
  This is the ONE exception to "do not add content" — the panel may only synthesize what the sections already state. TONE: analysis only — rewrite any row that assigns a task to a person/role or sets a deadline into the observable metric/release instead.
- ONE-PAGER LENGTH — the summary is a one-pager; ENFORCE it: if there are more than 5 highlights, keep the 5 most important; if a highlight exceeds 2 sentences / ~35 words, CUT it to its fact (figure + date) + one effect clause; if there are more than 3 Key Themes, keep 3 (at least one downside theme); themes longer than one sentence get trimmed to one. When cutting, preserve figures + attributions exactly — condense by dropping words, never by changing facts.
- Do NOT add new stories, themes, or facts (sole exception: the priorities panel above); do NOT rewrite prose that is already correct; keep the exact HTML structure (<p class="summary-prep">, the priorities panel, the sum-list items, the Key Themes block).
- If NOTHING needs correcting (panel present, budget respected, all figures/attributions match), output exactly OK (those two letters, nothing else) — do NOT re-emit the summary.
- Otherwise output ONLY the corrected HTML, starting with <p class="summary-prep">.`;
  try {
    const out = ((await complete(system, user, opts.maxTokens || 8000, usage, opts)) || '').trim();
    if (out === 'OK') return summaryHtml;              // clean bill — skip the expensive re-emit
...
and produce the Executive Summary block — a strict ONE-PAGER (~300 words of prose): the visual
priorities panel, the top 5 stories (1–2 sentences each), and exactly 3 cross-sectional Key Themes.
A CEO must grasp the day from this in under a minute; depth lives in the sections, not here.
Follow the synthesis rules below exactly. The HTML template in the user message is AUTHORITATIVE —
reproduce its structure verbatim, including the priorities panel.${langDirective(client)}
...
Produce the Executive Summary section content. THE SUMMARY IS A ONE-PAGER — a CEO must grasp the day in under a minute. Output ONLY this exact HTML structure — no markdown fences, no preamble:
...
<div style="margin:0 0 24px;border:1px solid var(--border,#E2DFD8);border-radius:12px;overflow:hidden">
  <div style="padding:9px 16px;background:var(--surface-hi,#F3F0EB);border-bottom:1px solid var(--border,#E2DFD8);font-family:'Inter',Arial,sans-serif;font-size:10px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:var(--text-3,#94A3B8)">[DE: PRIORITÄTEN HEUTE / EN: TODAY'S PRIORITIES]</div>
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse">
    <tr>
      <td width="4" style="background:[urgency colour — #C41E3A if action-relevant NOW, #D97706 this week, #94A3B8 ongoing]"></td>
      <td style="padding:10px 14px 10px 12px;border-bottom:1px solid var(--surface-hi,#F3F0EB)"><span style="font-family:'Playfair Display',Georgia,serif;font-size:16px;font-weight:800;color:var(--accent,#C41E3A)">1</span>&nbsp; <strong style="font-family:'Inter',Arial,sans-serif;font-size:13.5px;color:var(--text,#0F172A)">[Topic — MAX 4 words]</strong><div style="font-family:'Inter',Arial,sans-serif;font-size:11.5px;color:var(--text-2,#475569);margin-top:2px;line-height:1.5">[MAX 10 words: the concrete observable — metric, release, or date — that decides it]</div></td>
      <td align="right" style="padding:10px 14px;border-bottom:1px solid var(--surface-hi,#F3F0EB);vertical-align:middle;white-space:nowrap"><span style="display:inline-block;padding:3px 10px;border-radius:20px;font-family:'Inter',Arial,sans-serif;font-size:9.5px;font-weight:700;letter-spacing:0.06em;background:[#FEE2E2 hoch / #FEF3C7 mittel / #E2E8F0 niedrig];color:[#B91C1C hoch / #B45309 mittel / #475569 niedrig]">[DE: HOCH|MITTEL|NIEDRIG / EN: HIGH|MEDIUM|LOW]</span></td>
    </tr>
    [3–4 rows total, identical structure, ranked 1..n; the LAST row's two content <td>s use border-bottom:none]
  </table>
  <div style="padding:6px 16px 8px;font-family:'Inter',Arial,sans-serif;font-size:9px;color:var(--text-3,#94A3B8)">[DE: Farbleiste links = Dringlichkeit · Chip = Materialität / EN: Left bar = urgency · Chip = materiality]</div>
</div>
<ul class="sum-list">
  <li class="sum-item"><span class="sum-num">01</span><span class="sum-text"><strong>[Bold title, MAX 6 words]</strong> [1–2 sentences, MAX 35 words total: the fact with figure + date, then the client-relevant effect — hedged ("kann", "spricht für"), no third sentence.]</span></li>
  <li class="sum-item"><span class="sum-num">02</span><span class="sum-text"><strong>[Bold title]</strong> [1–2 sentences, ≤35 words.]</span></li>
  <li class="sum-item"><span class="sum-num">03</span><span class="sum-text"><strong>[Bold title]</strong> [1–2 sentences, ≤35 words.]</span></li>
  <li class="sum-item"><span class="sum-num">04</span><span class="sum-text"><strong>[Bold title]</strong> [1–2 sentences, ≤35 words.]</span></li>
  <li class="sum-item"><span class="sum-num">05</span><span class="sum-text"><strong>[Bold title]</strong> [1–2 sentences, ≤35 words.]</span></li>
</ul>
<div style="margin-top:24px;padding-top:18px;border-top:1px solid var(--border)">
  <h3 style="font-family:'Playfair Display',serif;font-size:16px;font-weight:700;color:var(--text);margin-bottom:10px">Key Themes</h3>
  <ul style="margin:0;padding-left:20px;font-size:13.5px;color:var(--text-2);line-height:1.65">
    <li style="margin-bottom:8px"><strong style="color:var(--text)">[Theme label, ≤3 words]</strong> — [ONE sentence, ≤20 words, connecting 2+ sections.]</li>
    <li style="margin-bottom:8px"><strong style="color:var(--text)">[Theme label]</strong> — [ONE sentence, ≤20 words.]</li>
    <li style="margin-bottom:8px"><strong style="color:var(--text)">[Downside theme — MANDATORY: the risk case]</strong> — [ONE sentence, ≤20 words.]</li>
  </ul>
</div>

ONE-PAGER BUDGET (hard limits): 3–4 priority rows (topic ≤4 words, observable ≤10 words) + EXACTLY 5 highlights (≤35 words each) + EXACTLY 3 Key Themes (≤20 words each). Total prose across the whole summary ≤ ~300 words. Depth lives in the sections — the summary is the map, not the territory.

PRIORITIES PANEL: rank 3–4 topics by MATERIALITY (expected financial impact for ${client.client_name}) — shown as the chip — × URGENCY (does it demand attention today / this week / ongoing?) — shown as the left colour bar. Draw only from the sections. The one-line observable names the metric, data release, threshold, or date that shows which way it breaks — telegram style, no filler verbs. ANALYST ROLE: never assign tasks, never address internal roles, never set deadlines, never a content-free "beobachten".

Rank the 5 highlights by Importance Score (highest = 01), each covering a DISTINCT story. EVIDENCE DISCIPLINE: fact first (figure + date), interpretation marked as interpretation — a market indicator (index, FX, rate move) is a sentiment signal, never direct evidence of an operational outcome like bookings or demand. ANTI-REPETITION: each underlying development appears in AT MOST ONE highlight; shared drivers consolidate into ONE Key Theme. ACCURACY: use ONLY figures, rates, and dates that already appear in the section outputs above — copy them exactly, keep each figure paired with ITS OWN attribution, and NEVER introduce a new or different number, rate, or date. No forward-looking conclusions of your own; preserve the sections' hedges. If a story has no verified figure, write the highlight without one rather than inventing it.
