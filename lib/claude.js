 ...
 */
export async function complete(systemPrompt, userPrompt, maxTokens = MAX_TOK, usage = null) {
  ...
  // Accumulate token usage for cost tracking when a collector is supplied.
  if (usage && msg.usage) {
...
// Pricing for the active model (claude-sonnet-4-6), USD per token.
...
function runSectionAgent(agentId, client, profile, searchResults, usage = null) {
  const id    = String(agentId).padStart(2, '0');
...
Use the ## Output format exactly: every story is an identical <article class="story-lead"> block, no wrappers.`;
  ...
  return complete(agentSystem(id, client), user, maxTokensFor(count), usage);
}

export const runAgent01 = (client, profile, sr, usage) => runSectionAgent(1, client, profile, sr, usage);
export const runAgent02 = (client, profile, sr, usage) => runSectionAgent(2, client, profile, sr, usage);
export const runAgent03 = (client, profile, sr, usage) => runSectionAgent(3, client, profile, sr, usage);
export const runAgent04 = (client, profile, sr, usage) => runSectionAgent(4, client, profile, sr, usage);
export const runAgent05 = (client, profile, sr, usage) => runSectionAgent(5, client, profile, sr, usage);
export const runAgent06 = (client, profile, sr, usage) => runSectionAgent(6, client, profile, sr, usage);
...
and produce the Executive Summary block for the briefing — an up-front bottom line (BLUF),
a ranked list of the top 7 stories (each a substantial 2–3 sentence analysis), and 4
cross-sectional Key Themes. Follow SKILL.md Sections 8 and 9 exactly.${langDirective(client)}
...
<p class="summary-bluf">[BOTTOM LINE — 2–3 sentences. The single most important takeaway across ALL sections today: what materially changed, the through-line connecting the day's signals, and why it matters for ${client.client_name}. Lead with the most decision-relevant number.]</p>
<ul class="sum-list">
...
Rank the 7 highlights by Importance Score (highest = 01). RULES: every highlight MUST be 2–3 full sentences (never a single clause), MUST contain at least one quantitative data point AND a date, and must cover a DISTINCT story (no repeats). The BLUF must not merely restate highlight 01 — it synthesises across the briefing.
