...
- PRIORITY BOX — the "Prioritäten heute"/"Today's priorities" <div> after summary-prep is a REQUIRED part of the format. If it is present: verify each topic and its materiality/urgency claim is grounded in a section. If it is MISSING: BUILD it yourself from the sections — 3–4 topics ranked by materiality × urgency for the client, using this exact structure:
  <div style="margin:0 0 26px;padding:16px 20px;border:1px solid var(--border,#E2DFD8);border-left:3px solid var(--accent,#C41E3A);border-radius:0 10px 10px 0;background:var(--surface-hi,#F3F0EB)"><h3 style="font-family:'Playfair Display',Georgia,serif;font-size:15px;font-weight:700;color:var(--text,#0F172A);margin:0 0 12px">[DE: Prioritäten heute — was Aufmerksamkeit verlangt / EN: Today's priorities]</h3><ol style="margin:0;padding-left:18px;font-size:13.5px;color:var(--text-2,#475569);line-height:1.7"><li style="margin-bottom:8px"><strong style="color:var(--text,#0F172A)">[Topic]</strong> — Materialität: [hoch/mittel] · Dringlichkeit: [hoch/mittel/laufend] → <em>[1 sentence decision-relevant analysis + the concrete observable]</em></li>…</ol></div>
  This is the ONE exception to "do not add content" — the box may only synthesize what the sections already state. TONE: analysis only — rewrite any row that assigns a task to a person/role or sets a deadline ("Treasury: … bis Freitag aufbereiten") into decision-relevant analysis naming the observable metric/release instead.
- Do NOT add new stories, themes, or facts (sole exception: the priority box above); do NOT rewrite prose that is already correct; keep the exact HTML structure (<p class="summary-prep">, the priority box, the sum-list items, the Key Themes block).
...
and produce the Executive Summary block for the briefing — the priorities box, a ranked list of the
top 7 stories (each a concise, CEO-level analysis of at most 3 sentences), plus 4–5 cross-sectional
Key Themes. Follow the synthesis rules below exactly. The HTML template in the user message is
AUTHORITATIVE — reproduce its structure verbatim, including the priorities box <div>.${langDirective(client)}
...
<div style="margin:0 0 26px;padding:16px 20px;border:1px solid var(--border,#E2DFD8);border-left:3px solid var(--accent,#C41E3A);border-radius:0 10px 10px 0;background:var(--surface-hi,#F3F0EB)">
  <h3 style="font-family:'Playfair Display',Georgia,serif;font-size:15px;font-weight:700;color:var(--text,#0F172A);margin:0 0 12px">[DE: "Prioritäten heute — was Aufmerksamkeit verlangt" / EN: "Today's priorities — what needs attention"]</h3>
  <ol style="margin:0;padding-left:18px;font-size:13.5px;color:var(--text-2,#475569);line-height:1.7">
    <li style="margin-bottom:8px"><strong style="color:var(--text,#0F172A)">[Topic, 2–5 words]</strong> — [DE: Materialität / EN: Materiality]: [hoch/mittel] · [DE: Dringlichkeit / EN: Urgency]: [hoch/mittel/laufend] → <em>[1 sentence: WHY this is decision-relevant now, plus the concrete observable — metric, release, or date — that will show which way it breaks. Analysis only: no task assignments, no roles, no deadlines.]</em></li>
    <li style="margin-bottom:8px">[2nd priority, same pattern]</li>
    <li style="margin-bottom:8px">[3rd priority, same pattern]</li>
    <li style="margin-bottom:8px">[4th priority — include only if genuinely decision-relevant today; 3–4 items total]</li>
  </ol>
</div>
<ul class="sum-list">
...
PRIORITY BOX (the <div> right after summary-prep): rank 3–4 topics by MATERIALITY (expected financial impact for ${client.client_name}) × URGENCY (does it require attention today/this week?). Draw only from the sections; each row ends with ONE sentence of decision-relevant analysis plus the concrete observable (metric, data release, threshold, or date) that will show which way it breaks — e.g. "Refinanzierungskosten variabler Tranchen sinken; entscheidend sind die Bankmargen der nächsten Kreditrunden". STAY IN THE ANALYST ROLE: never assign tasks, never address internal roles (no "Treasury:", "HR:"), never set deadlines ("bis Freitag") — and never a content-free "beobachten" without naming what. Write the box in the client's output language.

Rank the 7 highlights by Importance Score (highest = 01). RULES: every highlight is AT MOST 3 sentences — tight, information-dense, CEO-level prose (no filler) — and covers a DISTINCT story (no repeats). EVIDENCE DISCIPLINE: state the fact first (figure + date), then mark interpretation as interpretation ("kann", "spricht für") — a market indicator (index, FX, rate move) is a sentiment signal and must never be presented as direct evidence of an operational outcome like bookings or demand. ANTI-REPETITION: each underlying development (e.g. the ECB decision) appears in AT MOST ONE highlight; when several sections carry the same driver, consolidate its cross-sector meaning into ONE Key Theme instead of repeating it. ACCURACY: use ONLY figures, rates, and dates that already appear in the section outputs above (those have been fact-checked) — copy them exactly and NEVER introduce a new or different number, rate, or date. Keep each figure paired with ITS OWN attribution from the section — do not move a figure under a different source's name. Do NOT add forward-looking conclusions of your own (e.g. "Zinswende in Sicht", an expected rate cut): synthesize only what the sections state, with their hedges intact. Include a figure where the section provides one; if a story has no verified figure, write the highlight without one rather than inventing it. Produce 4–5 Key Themes, each a well-described bullet (bold label + 1–2 sentences) connecting at least 2 sections — at least ONE theme must be a downside/risk theme (what could go wrong across sections), not only opportunities.
