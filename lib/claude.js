...
// Load SKILL.md once at cold-start (it never changes at runtime)
const SKILL_MD = readFileSync(
  join(process.cwd(), 'Scheduled/morning-news-briefing/SKILL.md'),
  'utf-8'
);
...
function agentSystem(agentId, client) {
  return `You are the Intelio research system running AGENT ${String(agentId).padStart(2, '0')}.

## Core methodology (from SKILL.md)
${SKILL_MD}
...
CLIENT_LOCAL_SOURCES      = ${JSON.stringify(client.client_local_sources || [])}
STORIES_PER_SECTION       = ${client.stories_per_section || 3}
LOOKBACK_HOURS            = ${client.lookback_hours || 72}
CLIENT_PROFILE_REFRESH    = "${client.client_profile_refresh || 'monthly'}"

## Today's date
${new Date().toISOString().split('T')[0]}

## Your output
Produce ONLY valid HTML following the story-card format from Section 9 of the methodology.
Do NOT wrap in \`\`\`html\`\`\` fences. Start directly with <div class="bsec"> ...
`;
}
...
Follow the card format from SKILL.md Section 9 exactly.`;
  ...
  return complete(agentSystem('01', client), user);
}
...
Follow the card format from SKILL.md Section 9 exactly.`;
  ...
  return complete(agentSystem('02', client), user);
}
...
Follow the card format from SKILL.md Section 9 exactly.`;
  ...
  return complete(agentSystem('03', client), user);
}
...
Follow the card format from SKILL.md Section 9 exactly.`;
  ...
  return complete(agentSystem('04', client), user);
}
...
Follow the card format from SKILL.md Section 9 exactly.`;
  ...
  return complete(agentSystem('05', client), user);
}
...
Follow the card format from SKILL.md Section 9 exactly.`;
  ...
  return complete(agentSystem('06', client), user);
}
