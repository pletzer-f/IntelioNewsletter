...
// Extract only the story card format for section agents (sections 9.2–9.3)
// This keeps each agent call under ~1,500 tokens so 6 can run in parallel within 10K TPM
const CARD_FORMAT_START = SKILL_MD.indexOf('### 9.2 Story card format');
const CARD_FORMAT_END   = SKILL_MD.indexOf('## 10. QUALITY GATES');
const CARD_FORMAT = SKILL_MD.slice(CARD_FORMAT_START, CARD_FORMAT_END).trim();
...
function agentSystem(agentId, client) {
  return `You are Intelio AGENT ${String(agentId).padStart(2, '0')} — an economic intelligence analyst.
