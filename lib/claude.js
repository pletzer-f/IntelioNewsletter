...
DE: Signal / Evidenz / Risiko zur These / Nächster Schritt · EN: Signal / Evidence / Risk to thesis / Next step.]`;
 ...
 *  block (Signal/Evidenz/Risiko/Nächster Schritt + Beispielrechnung) adds
...
- INFORMATION GAP PROTOCOL — when the sources are too thin to cover a topic that is clearly central to this client${isReputationAgent ? ' (e.g. local permits, municipal decisions, project-specific coverage)' : ''}, do NOT pad and do NOT stop at "manual research recommended". Instead close the section with ONE compact protocol card (an <article class="story-lead"> whose story-body is a short list): WAS prüfen (name the concrete registers, municipalities, bodies, or projects from the client context), WO (the specific source or database), WIE OFT (cadence), WER (suggested owner role, e.g. Leiter Projektentwicklung), ESKALATION (what finding triggers action). No key-stat, no source link for this card.
...
- STRUCTURED IMPLICATION — the impl-inner labels (Signal / Evidenz|Evidence / Risiko zur These|Risk to thesis / Nächster Schritt|Next step) are part of the required format: keep them. Check the Signal tag is honest — an index/FX/rate move tagged "direkt operativ" must be corrected to "indirekt (Makro)". An INFORMATION-GAP protocol card (a story listing what/where/how often/who to check, with no key-stat and no source link) is legitimate — do not delete it for lacking sources.
...
<div style="margin:0 0 26px;padding:16px 20px;border:1px solid var(--border,#E2DFD8);border-left:3px solid var(--accent,#C41E3A);border-radius:0 10px 10px 0;background:var(--surface-hi,#F3F0EB)">
  ...
  <ol style="margin:0;padding-left:18px;font-size:13.5px;color:var(--text-2,#475569);line-height:1.7">
    <li style="margin-bottom:8px"><strong style="color:var(--text,#0F172A)">[Topic, 2–5 words]</strong> — [DE: Materialität / EN: Materiality]: [hoch/mittel] · [DE: Dringlichkeit / EN: Urgency]: [hoch/mittel/laufend] → <em>[one specific recommended action or management question, addressed to a role]</em></li>
...
PRIORITY BOX (the <div> right after summary-prep): rank 3–4 topics by MATERIALITY (expected financial impact for ${client.client_name}) × URGENCY (does it require attention today/this week?). Draw only from the sections; each row ends with one concrete action or management question addressed to a role (CFO, Treasury, Vertrieb iDM, Projektentwicklung …) — never a bare "beobachten". Write the box in the client's output language.
