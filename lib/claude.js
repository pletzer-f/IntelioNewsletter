...
export const runAgent07 = (client, profile, sr, usage) => runSectionAgent(7, client, profile, sr, usage);
...
Follow SKILL.md Sections 8 and 9 exactly.${langDirective(client)}
...
Rank the 7 highlights by Importance Score (highest = 01). RULES: every highlight is AT MOST 3 sentences — tight, information-dense, CEO-level prose (no filler) — MUST contain at least one quantitative data point AND a date, and must cover a DISTINCT story (no repeats). Produce 4–5 Key Themes, each a well-described bullet (bold label + 1–2 sentences) connecting at least 2 sections.
