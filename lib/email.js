...
// Sender address. Defaults to the verified 8th-universe.com Resend domain so sends
// work to any recipient out of the box; override per-tenant via the RESEND_FROM env var.
const FROM_ADDRESS = process.env.RESEND_FROM || 'Intelio <noreply@8th-universe.com>';
...
<style>
  ...
  .summary-area .summary-bluf {
    font-family: Arial, sans-serif; font-size: 15px; line-height: 1.7; color: #1E293B;
    font-weight: 500; margin: 0 0 20px; padding: 2px 0 2px 14px; border-left: 3px solid #C41E3A;
  }
  .summary-area .summary-bluf strong { color: #0F172A; font-weight: 700; }
...
<div class="shell">
  ...
  try {
    ...
    throw new Error(`Resend send failed (from="${FROM_ADDRESS}", to="${toEmail}"): ${e?.message || e}`);
  }
  if (res?.error) throw new Error(`Resend error: ${JSON.stringify(res.error)}`);
  return res?.data;
}
...
export async function sendTransactional(toEmail, subject, html) {
  let res;
  try {
    res = await resend.emails.send({ from: FROM_ADDRESS, to: toEmail, subject, html });
  } catch (e) {
    // The SDK throws synchronously on an invalid from/to address format, with an
    // opaque message ("The string did not match the expected pattern."). Wrap it.
    throw new Error(`Resend send failed (from="${FROM_ADDRESS}", to="${toEmail}"): ${e?.message || e}`);
  }
  if (res?.error) throw new Error(`Resend error: ${JSON.stringify(res.error)}`);
  return res?.data;
