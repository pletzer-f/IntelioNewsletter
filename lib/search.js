 ...
 * @param {string} [opts.country='DE'] — localise results
 * @param {string} [opts.freshness]       — e.g. 'pd' (past day), 'pw' (past week)
 * @returns {Promise<BraveResult[]>}
 */
export async function search(query, { count = 10, country = 'DE', freshness } = {}) {
 ...
 * Uses allSettled so a single failed query (rate limit, timeout) doesn't abort all results.
 * @param {string[]} queries
 * @param {object}   opts  — same as search()
 * @returns {Promise<BraveResult[]>}
 */
export async function multiSearch(queries, opts = {}) {
  const settled = await Promise.allSettled(queries.map(q => search(q, opts)));

  // Log any individual failures but don't abort — partial results are better than none
  settled.forEach((r, i) => {
    if (r.status === 'rejected') {
      console.warn(`[search] Query ${i} failed (continuing): ${r.reason?.message}`);
    }
  });
