...
export async function upsertClient(config) {
  ...
  const rawSections = (config.SECTIONS_ENABLED || [1,2,3,4,5,6,7]).map(v => {
    const n = Number(v);
    if (!isNaN(n) && n > 0) return n;            // numeric or numeric string
    return SLUG_TO_NUM[v] ?? null;               // slug string or unknown
  }).filter(n => n !== null);
  const sectionsEnabled = rawSections.length > 0 ? rawSections : [1,2,3,4,5,6,7];
  ...
  // delivery_dow: 0=Sun..6=Sat (weekday for weekly briefings; ignored for daily). Default Monday.
  const deliveryDow  = (() => { const d = Number(config.DELIVERY_DOW); return (Number.isInteger(d) && d >= 0 && d <= 6) ? d : 1; })();
  ...
  const { data, error } = await supabase
    ...
    .upsert({
      ...
      delivery_dow:            deliveryDow,
      client_profile_refresh:  config.CLIENT_PROFILE_REFRESH,
      stories_per_section:     config.STORIES_PER_SECTION,
      lookback_hours:          config.LOOKBACK_HOURS,
      // Default true (signup); callers updating an existing client pass the
      // current value so a preference save can't re-activate a paused client.
...
export async function saveBriefing(clientId, html, date, inputTokens = null, outputTokens = null, costUsd = null) {
  const row = {
    client_id:  clientId,
    html,
    date,
  ...
  // Persist token usage + real cost when supplied (powers the admin cost dashboard).
  if (inputTokens  != null) row.input_tokens  = inputTokens;
  if (outputTokens != null) row.output_tokens = outputTokens;
  if (costUsd      != null) row.cost_usd       = costUsd;

  const { data, error } = await supabase
    .from('briefings')
    .upsert(row, { onConflict: 'client_id,date' })   // overwrite today's briefing on re-run
    .select()
    .single();
  if (error) throw error;
  return data;
